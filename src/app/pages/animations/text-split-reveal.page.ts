    ...
    <div class="lg:grid lg:grid-cols-[1fr_16rem] lg:gap-10">
      <div class="space-y-16">
        <div id="demo" class="scroll-mt-28">
          <app-recipe-demo number="03" title="Demo">
        ...
        <app-recipe-section title="How it works" icon="🔍" id="how-it-works">
          <p>
            ...
            <code>computed()</code> signal — <code>text().split(' ')</code> —
            and each word is rendered by its own <code>&#64;for</code> loop
            iteration wrapped in an <code>inline-block</code> span. A
            <code>viewChildren</code> query collects those spans, and a single
            <code>gsap.from()</code> call animates them all with a small
            stagger, sliding each word up from below its own baseline.
          </p>
          <p>
        ...
        <app-recipe-section title="The Angular way" icon="🅰️" id="angular-way">
          <ul>
            <li>
              ...
              <code>text</code> input, so splitting stays declarative and
              re-runs automatically if the input ever changes.
            </li>
            <li>
            ...
            <li>
              <code>DestroyRef.onDestroy()</code> kills the tween so replaying
              the animation, or navigating away mid-animation, never leaves two
              tweens fighting over the same spans.
            </li>
          </ul>
        </app-recipe-section>
        ...
        <app-recipe-section title="Common pitfalls" icon="⚠️" id="pitfalls">
          <ul>
            <li>
              ...
              <code>aria-label</code> on the container — screen readers will
              read the sentence one word (or letter) at a time.
            </li>
            <li>
              Splitting on every render instead of via a <code>computed()</code>
              — recomputing the word array on unrelated change detection runs
              can tear down and rebuild the DOM nodes GSAP is animating.
            </li>
            <li>
              Animating with percentage-based <code>y</code> values on an
              element that isn't <code>display: inline-block</code> (or block) —
